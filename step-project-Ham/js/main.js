let tab = function () {
    let tabTitle = document.querySelectorAll('.tab_title')
...
const imgOut = document.querySelector('.work_tabs_picture');
const liTab = document.querySelectorAll('.work_tabs_title');
const loadMoreBtn = document.querySelector('.loadMore');
const loader = document.querySelector('.loader');
let tabName;
let category;

const ourAmazingWorkList = tabsArr.reduce((acc, item) => {
    acc[item.category] = Array.isArray(acc[item.category]) ? [...acc[item.category], item] : [item];
    return acc;
}, {});
...
const hoverLayout = `
    <div class="hover">
        <div class="hover-icon">
            <a href="#"><svg width="88" height="43"
                            viewBox="0 0 88 43" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="hover-span-text">
            <span class="hover-span-title">creative design</span>
            <span class="hover-span-info">Web Design</span>
        </div>
    </div>
`
...
const renderImage = (item) => {
    ...
    img.src = item.src
    box.innerHTML = hoverLayout
    box.append(img)
    imgOut.append(box)
}

function setImgCategory(name, offset = [0, 10]) {
    imgOut.innerHTML = '';
    category = name;
    if (name !== 'all') {
        ourAmazingWorkList[category].slice(offset[0], offset[1]).forEach(item => {
            renderImage(item)
        })
    } else {
        tabsArr.slice(offset[0], offset[1]).forEach(item => {
            renderImage(item)
        })
    }
...
let offset = [0, 12];
liTab.forEach(item => item.addEventListener('click', selectTab))
...
function selectTab() {
    ...
    setImgCategory(tabName, offset)
}

setImgCategory('all', offset)

loadMoreBtn.addEventListener('click', () => {
    loader.classList.add('active');
    offset = [0, offset[1] + 12]
    setTimeout(() => setImgCategory(category, offset), 3000);
})
